# 03 — Agent State Semantics, Access Control, and Trust Model (V2)
...
*   **Integrity Guarantee:** Models may inspect $\mathcal{S}_{\text{auth}}$ to plan actions, but they are physically isolated from writing to $\mathcal{S}_{\text{auth}}$. Transitions are driven exclusively by external authenticated principals or deterministic reducers.
