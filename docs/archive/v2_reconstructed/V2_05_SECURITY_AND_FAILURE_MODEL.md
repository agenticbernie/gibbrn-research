# 05 — Security Architecture, Threat Model, and Failure Modes (V2)
...
*   **gibbrn Hardened Defense:**
    ...
    - **Filesystem Chroot & Seccomp:** The micro-sandbox mounts an isolated ephemeral workspace. Host credentials (`~/.aws`, `~/.ssh`, `/etc`) are physically unmapped from the container's mount table, rendering credential theft impossible regardless of the script executed.
...
*   **gibbrn Hardened Defense:**
    - **Invariant 1 Enforcement:** The foundation model is physically incapable of writing to Authoritative State ($\mathcal{S}_{\text{auth}}$).
...
*   **Attack Mechanism:** Attackers inject malicious guidance into the agent's long-term retrieval storage (vector database) via query-only interactions (Dong et al., NeurIPS 2024 [1]). Weeks later, the agent retrieves the poisoned record during an unrelated task and executes harmful instructions.
*   **gibbrn Hardened Defense:**
