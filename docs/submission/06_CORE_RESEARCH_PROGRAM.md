...
ARC III (M25–M36, CONDITIONAL) — MULTI-AGENT & ENVIRONMENT CONTINUITY
  ...
  RQ9:  Decision-Sufficient Canonical Operational State
...
## 2. Formal Metric Proposal: Maximum Dependable Interaction Depth ($\text{MDID}_\tau$)

The dossier formally introduces $\text{MDID}_\tau$ as a **proposed research metric**, abandoning the naive geometric compounding assumption ($P = p^d$).

> **V4.1 naming correction:** earlier dossier versions called this quantity Maximum Dependable *Dependency* Depth. The implementation measures an interaction-step index, not a causal-dependency path length — the Causal State Spine records dependency structure, but depth here advances once per agent–environment interaction cycle. The honest name is Maximum Dependable *Interaction* Depth. A true causal-path depth (longest dependency path from trajectory root to the current consequential effect) remains future instrumentation, not a claimed metric. Pre-V4 documents (verification ledgers, diligence audit, LICENSE illustration) use MDID's former name for the same interaction-index quantity; they are historical and are not rewritten.

### 2.1 The Competing-Risks Framework (V4.2 — KM Contradiction Resolved)
Let each trajectory terminate in fatal failure, task completion, or budget-exhaustion censoring. The framework below estimates both incidence curves jointly.
...
- *Interaction depth:* the step index $k$ along the executed trajectory (not the count of useful steps). Padding incentives are addressed — without being assumed away — by the padding defense below.
...
- *Recovery:* return to a prior valid checkpoint $C_j$ followed by eventual task completion within budget. Recovery counts are reported separately and do not reset interaction depth.
- *Completion (competing event, first-class):* task goal reached per the to-be-preregistered verifier before failure. Completion is an informative competing event — not independent censoring. It is analyzed jointly with failure (see competing-risks framework below), and its probability is a co-primary report at M18.
...
> **V4.2 correction:** V4.1 stated completion is "NOT censored" while still defining MDID through a Kaplan-Meier estimator whose risk set excluded completions — an incoherent estimator. Under competing risks, naïve KM does not estimate any absolute incidence correctly. The block below replaces the KM-based depth definition with a CIF-based one. There is exactly one binding depth estimand, and it lives inside the competing-risks machinery.

Each trajectory ends in exactly one terminal state: **fatal failure** (event of interest), **task completion** (competing event), or **budget exhaustion** (right-censoring — the only censored outcome, assumed independent of the failure hazard as an exogenous cap).

1.  **Cause-specific counting:** at each interaction depth $k$, let $d_{1,k}$ be fatal failures, $d_{2,k}$ completions, $c_k$ budget-censorings, and $n_k$ the trajectories still at risk (no terminal event yet) just prior to $k$.
2.  **Cumulative incidence (Aalen–Johansen):** the fatal-failure CIF and completion CIF are estimated jointly:
    $$\hat{F}_1(k) = \sum_{i \le k} \hat{S}(i-1) \cdot \frac{d_{1,i}}{n_i}, \qquad \hat{F}_2(k) = \sum_{i \le k} \hat{S}(i-1) \cdot \frac{d_{2,i}}{n_i}$$
    where $\hat{S}(i-1)$ is the overall event-free survival just prior to $i$. Completion is a legitimate terminal state inside the estimator — never pseudo-censoring.
3.  **Definition of $\text{MDID}_\tau$ (binding):** the maximum interaction depth $k$ at which the estimated fatal-failure-free probability — one minus the fatal-failure CIF — remains at or above $\tau \in (0, 1]$ (standard baseline $\tau = 0.90$):
    $$\text{MDID}_\tau = \max \left\{ k \in \mathbb{N} \;\middle|\; 1 - \hat{F}_1(k) \ge \tau \right\}$$
    with bootstrap confidence intervals for arm-wise MDID and for the MDID ratio. The quantity $1 - \hat{F}_1(k)$ is the probability of surviving past $k$ *without fatal failure* (completion allowed) — the operationally meaningful "still running safely" curve.

*Where the old KM quantity went:* a cause-specific Kaplan-Meier curve that censors completions is retained **only as a labeled sensitivity analysis** ("what the depth curve would look like under the acknowledged-false independent-censoring assumption"). It is never binding, never gated, and never reported without the CIF-based primary beside it. Table 6.2's MDID criterion refers exclusively to the CIF-based definition above.

*   **Primary efficacy analysis (competing risks, first-class):** Gray's test on the fatal-failure CIF ($\alpha = 0.01$) for *whether* incidence differs; the **primary magnitude estimand** is the absolute fatal-failure CIF reduction at a fixed pre-registered interaction horizon $k^*$:
    $$\Delta F_1(k^*) = \hat{F}_{1,B}(k^*) - \hat{F}_{1,C}(k^*)$$
    with a two-sided 95% confidence interval (bootstrap or Aalen–Johansen variance, frozen at pre-registration). The numeric pass threshold for $\Delta F_1(k^*)$ is **TBD at pilot calibration** — no interim number is quoted, not even as a planning assumption, to remove all researcher degrees of freedom. The Fine-Gray subdistribution hazard ratio is a secondary, model-based support statistic — never an alternative gate to shop between.
*   **M18 joint verdict (five quantities, C vs B):** completion CIF level; fatal-failure CIF comparison (Gray's significance + $\Delta F_1(k^*)$ magnitude floor); CIF-based MDID ratio; cost; latency (see Table 6.2). No single metric decides the scientific thesis gate.

> **Censoring Assumption (V4.2):** only budget exhaustion is censored. Task completion is a competing terminal event estimated first-class via Aalen–Johansen. The estimator, the risk set, and the verdict rules are now mutually consistent.
...
*   **Primary Research Question:** Does physically separating consequential agent state into Cognitive, Operational, Authoritative, and Runtime tiers with distinct mutation semantics reduce state corruption without degrading task performance?
*   **Secondary Feasibility Question (non-binding instrumentation, not gated):** Can persistent-identity lineage and Goal Contract semantics be represented inside the taxonomy without violating its mutation rules (i.e., lineage/contract records live in canonical state, writable only by deterministic reducers or the root authority, readable as planning context)? Scored for schema validity and rule-conformance; no numeric threshold, no kill power.
...
*   **V4.1 note:** the consequential/non-consequential classification rubric is piloted alongside the secondary feasibility question above — descriptive only at M3.

---

### Core RQ2 — Causal Failure Reconstruction (binding) + Continuity Lineage (secondary)
*   **Binding Research Question:** Can typed, provenance-preserving state reconstruct causal failure lineage better than conventional telemetry?
*   **Secondary Research Question (non-binding at M6; informs RQ7/RQ8 design):** Can the same spine reconstruct identity, adaptation, objective, and authority lineage on purpose-built or synthetically instrumented continuity traces? AgentErrorBench natively lacks ground truth for Goal Contracts, IAM delegation, and identity migration, so no claim about those dimensions is tested on AgentErrorBench itself.
*   **Secondary metric — Lineage Reconstruction Accuracy (LRA, descriptive at M6):** per-dimension accuracy (identity / objective / authority / adaptation) at recovering the planted lineage event on instrumented traces with known ground truth. Reported with Wilson intervals; thresholds TBD at RQ7/RQ8 pre-registration — M6 publishes the instrument and the descriptive result, nothing more.
...
*   **V4 note:** lineage dimensions beyond causal failure (identity, adaptation, objective, authority) are measured only via the secondary LRA instrument on purpose-built traces at M6; the binding gate remains causal reconstruction on AgentErrorBench.
...
*   **Workload:** $N = 1,000$ adversarial attack trajectories (`gibbrn-auth-bench`, internal benchmark deliverable constructed during Phase 3) targeting file exfiltration, unauthorized network requests, and budget exhaustion — extended in V4 with endpoint-substitution, credential-rebound, and consequence-mismatch families. Pre-registration MUST fix per-family minimum quotas across all eight threat families (authority laundering; parameter smuggling; endpoint substitution; unauthorized network destination selection; credential rebound; action semantic drift; TOCTOU races; consequence mismatch) so that no family is left with so few samples that its family-specific bound is vacuous, and the M9 package MUST report per-family failure rates with per-family Clopper-Pearson bounds alongside the aggregate UER. An aggregate UER of 0 with an under-sampled family supports no claim about that family.
*   **Deterministic fault-injection axis (V4.1, alongside prompt-injection attacks):** security-context discontinuity MUST also be tested by direct fault injection at pipeline transitions, not only via model prompts: recipient substitution; resource-ID swap; endpoint rebound; credential substitution; scope widening; adapter-drops-delegation-field; authorized-$100 → realized-$1,000 amount mutation. Each injected fault carries a pre-registered expected verdict (fail-closed, or narrowed-pass with receipt); a silent widening or mismatched realized effect counts as a failure in the same UER accounting. This axis is what actually exercises consequence-integrity semantics. Attack variants are correlated (shared harness, overlapping payload families); effective independent information is less than $N$. Representativeness is limited to the covered attack families.
...
*   **Secondary abstraction metrics (V4.1 — descriptive at M12, binding only where stated):** FPR/retention prove safe *admission*, not useful *abstraction*. M12 MUST additionally publish, without kill power: Reusable Skill Yield (promoted families per 100 episodes); Held-out Family Transfer (success delta when a promoted family is applied to unseen tasks of the same family vs. no-skill baseline); Compression / library growth (families vs. per-task entries over time); False family merge rate and False family split rate (adjudicated against a frozen family-equivalence rubric); Benefit vs. per-task skill storage (head-to-head retained-skill ablation); Admission rate (fraction of candidates reaching evaluation, for selection-bias visibility). **Division of labor: M12 proves safe admission; M15 (RQ5 ATR) proves useful abstraction and transfer.** A family that is safe but transfers nothing is a documented null result, not a pass on abstraction.
...
*   **Research Question:** Does managed autonomy (autonomous agent + gibbrn state checkpoints and causal rollback) achieve a statistically significant improvement in the failure–completion–depth profile over deep interaction chains compared to conventional controls and static pipelines?
*   **V4 extension:** failure taxonomy expanded to epistemic drift, authority failure, objective drift, adaptation regression, runtime failure, migration discontinuity, and consequence mismatch. Arms: (A) unmanaged adaptive agent (secondary reference); (B) conventionally controlled agent (canonical baseline); (C) GIBBRN-controlled agent; and where appropriate (S) static deterministic pipeline.
*   **Hypothesis ($\mathcal{H}_1$):** On complex tasks requiring exploratory branching ($d \ge 30$), GIBBRN arm (C) versus the conventional-controls arm (B): (i) CIF-based $\text{MDID}_{0.90}(\text{C}) \ge 2.0 \times \text{MDID}_{0.90}(\text{B})$ with a bootstrap 95% confidence interval for the MDID ratio whose lower bound exceeds $1.5\times$; AND (ii) the fatal-failure CIF is significantly lower (Gray's test $p < 0.01$) with the primary magnitude estimand $\Delta F_1(k^*)$ meeting its pre-registered floor (numeric floor TBD at pilot calibration — no interim number quoted); AND (iii) completion CIF non-inferior (within $5\text{pp}$ of the best baseline arm at the pre-registered horizon); AND (iv) mean cost per completed task plus latency (median + p95) against proposed practicality triggers (pending pilot calibration — tripping a trigger yields INCONCLUSIVE, never a direct fail; see Table 6.2).
*   **Null Hypothesis ($\mathcal{H}_0$):** gibbrn checkpointing and causal rollback do not jointly improve the failure–completion–depth profile versus conventional controls, or static deterministic pipelines (Agentless / pre-wired DAGs) achieve equal or superior completion rates at lower compute cost.
*   **Three-arm comparison:** (A) unmanaged adaptive agent (secondary reference — problem severity only); (B) the same agent with conventional controls (scoped credentials, sandbox, durable execution, audit logs) but without gibbrn state-integrity mechanisms (**the canonical baseline**); (C) the same agent with gibbrn. The thesis requires (C) to beat (B), not only (A). Model, task pool, token/step budget, and run conditions are held equivalent where feasible.
...
*   **Statistical Methodology:** Competing-risks primary on Aalen–Johansen CIFs: Gray's test on fatal-failure CIF ($\alpha = 0.01$); primary magnitude $\Delta F_1(k^*)$ with 95% interval; completion CIF reported jointly; CIF-based MDID quantile + bootstrap CI for the MDID ratio (C vs B). Fine-Gray sHR is secondary model-based support only. Labeled sensitivity: cause-specific KM + Log-Rank under the acknowledged-false independent-censoring assumption. The Gray's-test $p$-value tests *whether failure incidence differs*; *magnitude* claims (≥2.0× depth, $\Delta F_1(k^*)$ floor) are evaluated via their intervals/estimates, never via any $p$-value, and never via a choice between alternative estimands.
*   **Failure Threshold at Gate M18:** Verdicts follow canonical Table 6.2 — this summary restates it without modification: joint PROCEED requires the CIF-based MDID ratio criterion AND Gray's significance with the $\Delta F_1(k^*)$ floor met AND completion non-inferiority AND no tripped practicality trigger (all C vs B). Partial satisfaction (e.g. depth met but $\Delta F_1(k^*)$ floor missed or straddled, or any straddling interval) → INCONCLUSIVE (more tasks/optimization); a tripped practicality trigger without a cost-reduction path → INCONCLUSIVE, not a pass. STOP/PIVOT only for: MDID ratio point <1.5, or CI upper bound <2.0 with adequate power, or no significant CIF reduction with adequate power, or completion inferior by $>5\text{pp}$, or static-pipeline win on the joint success–cost criterion. Incremental value is measured as (C)−(B), never (C)−(A) alone. (A) unmanaged and (S) static pipeline are secondary reports.
...
*   **Post-migration semantic conformance probes (V4.1 — record-copying is not continuity):** preservation rates above can be trivially gamed by copying a database. Every migration pair MUST therefore pass pre/post semantic probes: same authorized operation → ALLOW before and after; same forbidden operation → DENY before and after; narrowed scope → former capability now DENIED; pending commitment → retained or cancelled exactly per contract; verified skill → still passes its validation suite in the new runtime. A migrated agent with perfect record preservation that fails its probes has failed continuity.
*   **Negative migration cases (V4.1):** not every migration should succeed. The matrix MUST include incompatible pairs (e.g. skill bound to an unavailable runtime capability; contract requiring an authority the target host cannot anchor), where the correct outcome is safe refusal or explicit rebinding — never silent success. An agent that "migrates" an unanchorable authority without complaint fails RQ7.
...
*   **Two separable sub-questions (V4.1 — an intact contract does not imply faithful operationalization):**
    - **Contract Integrity:** was the canonical Goal Contract modified without root authorization? (Unauthorized redefinitions = failures, full stop.)
    - **Operationalization Fidelity (the hard problem):** do the agent's derived metrics, curricula, and subgoals actually track the principal's intended outcome? Canonical trap case: contract says "improve software reliability"; the agent defines reliability as "number of passing self-generated tests," writes easy tests, scores 100%, and changes nothing about real reliability. The contract is intact — objective integrity still failed. Fidelity MUST be measured against independent held-out outcomes the agent neither selected nor can game (frozen principal-authored acceptance probes), via proxy-divergence scoring (agent-optimized metric vs. contract acceptance criteria) and transfer checks (gains on self-made curricula must survive on hidden evaluation).
*   **Workload (planning sketch, TBD at pre-registration):** broad-objective task families where the agent must generate its own subgoals/curricula/evaluation criteria from a Goal Contract; red-team injections attempt contract redefinition, evaluator mutation, and leakage — plus proxy-gaming tasks instrumented with the trap pattern above (easy-metric satisfaction vs. true-outcome measurement).
*   **Candidate metrics (thresholds TBD at pilot calibration):** contract-violation rate (unauthorized redefinitions); proxy-divergence score (contract acceptance criteria vs. agent-optimized metric); operationalization-fidelity transfer (self-curriculum gains surviving hidden evaluation); leakage/contamination detection rate; hidden-regression rate on frozen holdouts; evaluator-mutation detection rate.
...
### Core RQ9 — Decision-Sufficient Canonical Operational State
...
*   **Scope note (V4.1):** "World" is deliberately absent from the core title. GIBBRN does not train world models and Year 3 does not expand into robotics. Puffin-World / JEPA-family findings serve strictly as a motivating analogy — physical prediction-sufficient ≠ decision-sufficient motivates the hypothesis that observed software state ≠ decision-sufficient operational state — never as architectural precedent.
...
*   **Verdict (V4.1 — same gate, two independent verdicts):** good science ≠ good company, and a good wedge ≠ every Year-3 hypothesis succeeding.
    - **M36-A — Scientific Verdict:** does the integrated continuity/integrity thesis hold on the composed metrics?
    - **M36-B — Company Verdict:** do developers/design partners obtain sufficient ROI, adoption value, and integration benefit?
    Either verdict can fail without the other: M36-A pass + M36-B fail means real research without a company; M36-A fail + M36-B pass means a useful wedge product without the grand thesis. The split exists so that, before any capital costing, scientific research, engineering/prototype, design-partner validation, and commercialization can be costed as separate lines.
...
| **Core RQ3** | gibbrn-auth-bench Red-Team (+ deterministic fault-injection axis) | 1,000 attacks (per-family minimum quotas frozen at pre-reg) | Fisher's Exact Test | 0.95 | 0.001 | Dynamic injection string randomization; per-family quotas + bounds |
...
| **Core RQ6** | GAIA Level 3 & Deep SWE-bench | 150 deep tasks (planning seed; final N from simulation) | Gray's CIF Test ($\Delta F_1(k^*)$ primary magnitude) | TBD via competing-risk simulation | 0.01 | Budget-cap censoring only; completion as competing event |
...
> **Note on Statistical Power:** Power values in Table 6.1 (RQ1–RQ5) are *preliminary design-time estimates*, not results of completed simulation-based power analyses. Assumed effect sizes: RQ1 corruption reduction 80% vs. 50% null; RQ2 CRR 80% vs. 45% baseline; RQ3 UER 0.000 vs. 0.25 control rate; RQ4 retention 98% vs. 80% degraded control; RQ5 ATR 0.80 vs. 0.50 null with 100 pairs; RQ6 power and final N are TBD via competing-risk simulation over baseline completion/failure incidence, budget-exhaustion rates, CIF differences, N, and arm allocation (the legacy 0.90/log-rank basis does not transfer to the joint gate; 150 tasks × 3 arms may be underpowered for simultaneous criteria). RQ7 pilot exceedance vs. calibrated preservation thresholds. Formal simulation-based power curves (with seeds, model versions, and censoring assumptions) will be generated, archived, and frozen as part of pre-registration *before* data collection at each milestone phase. Until then, sample sizes are planning targets. **For RQ7–RQ12, even planning effect sizes are withheld until pilot calibration — the table states this explicitly rather than inventing numbers.**
...
| M18 (RQ6) | JOINT over C-vs-B, all CIF-based: MDID ratio point ≥2.0 AND bootstrap CI lower >1.5 AND Gray's-test CIF reduction $p<0.01$ with the pre-registered $\Delta F_1(k^*)$ floor met AND completion within 5pp of best baseline arm AND no practicality trigger tripped (proposed triggers, pending pilot calibration — NOT binding fail: cost per completed task >3× best baseline, or p95 latency >2× baseline) | Any single joint element in its planning band with the rest met (e.g. ratio in [1.5,2.0) with interval including 2.0; Gray's significant but $\Delta F_1(k^*)$ floor straddled; any straddling interval) → more tasks/optimization; or ratio + CIF met but a practicality trigger tripped → cost/latency-reduction cycle + calibrate | MDID CI upper bound <2.0 with adequate power (interval entirely below target — e.g. 1.3× [1.1;1.6] fails, while 2.5× [2.1;2.9], which also excludes 2.0, is PROCEED on the depth element), or MDID ratio point <1.5, or no significant CIF reduction with adequate power, or completion inferior >5pp, or static pipeline wins joint success–cost → STOP/PIVOT |
...
| M36-A (RQ12 scientific) | Composed surviving-metric targets met (frozen at M33 pre-reg) → thesis PROCEED | Partial composition → NARROW to responsive vertical | No integrated advantage → thesis PIVOT or STOP per investor governance |
| M36-B (RQ12 company) | ROI/adoption/integration-benefit targets met (frozen at M33 pre-reg) → company PROCEED | Mixed partner ROI (1 of 2 positive) → NARROW to responsive vertical | No ROI and no adoption path → company PIVOT or STOP per investor governance. Independent of M36-A. |
...
| M9 | Attack suite manifest with per-family quotas, per-attack verdicts (prompt-injection + fault-injection axes), per-family and aggregate Clopper-Pearson/Wilson bounds, FDR/latency distributions, root-cause memos for any failure |
...
| M18 | Fatal-failure + completion CIFs (Aalen–Johansen), Gray's + $\Delta F_1(k^*)$ interval + Fine-Gray support, CIF-based MDID CIs, joint completion/depth/cost/latency table, no-op audit summary, simulation-based power report |
...
| M36 | Integrated deployment report, composed metrics, M36-A scientific verdict; partner ROI/adoption/integration logs, M36-B company verdict |
