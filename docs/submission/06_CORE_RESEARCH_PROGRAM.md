...
3.  **Definition of $\text{MDID}_\tau$:** The maximum sequential step depth $k$ at which the estimated trajectory survival probability remains at or above threshold $\tau \in (0, 1]$ (standard baseline $\tau = 0.90$):
    $$\text{MDID}_\tau = \max \left\{ k \in \mathbb{N} \;\middle|\; \hat{S}(k) \ge \tau \right\}$$
...
- $n_i$ is the number of trajectories at risk just prior to step $i$ (excluding right-censored tasks that hit budget limits; completions are handled by the competing-risks framework below, not by censoring).
- **Primary efficacy analysis (competing risks, first-class):** fatal failure and task completion are modeled as competing events. The primary comparison is the cumulative incidence function (CIF) of fatal failure across arms via Gray's test with target significance $\alpha = 0.01$, with effect magnitude from the Fine-Gray subdistribution hazard model. MDID$_{0.90}$ (Kaplan-Meier depth quantile) is retained as a descriptive, interpretable depth summary — it does not alone decide the thesis. The M18 verdict is joint over five quantities: completion probability; fatal-failure CIF (level + comparison); interaction-depth survival (MDID ratio); cost; latency (see Table 6.2). No single metric decides the scientific thesis gate.
- **Sensitivity analysis:** Log-Rank (Mantel-Cox) comparison of Kaplan-Meier curves is retained as a secondary sensitivity under the (acknowledged-strong) non-informative-censoring assumption, to show how conclusions move when completion is treated as censoring rather than competition.

> **Censoring Assumption (V4.1):** only budget exhaustion is treated as right-censored, under the assumption that hitting an exogenous cap is independent of the failure hazard. Task completion is NOT censored — it is a competing event analyzed first-class (Gray's test + Fine-Gray above). The Kaplan-Meier MDID quantile remains valid as a descriptive depth summary; it is not the efficacy estimand.
...
*   **Hypothesis ($\mathcal{H}_1$):** On complex tasks requiring exploratory branching ($d \ge 30$), gibbrn extends dependable operation such that, GIBBRN arm (C) versus the conventional-controls arm (B): (i) $\text{MDID}_{0.90}(\text{C}) \ge 2.0 \times \text{MDID}_{0.90}(\text{B})$ with a bootstrap 95% confidence interval for the MDID ratio whose lower bound exceeds $1.5\times$; AND (ii) the fatal-failure cumulative incidence is significantly lower (Gray's test $p < 0.01$) with magnitude meeting a pre-registered minimum effect (planning assumption, frozen at calibration: subdistribution hazard ratio $\le 0.67$ or CIF absolute reduction $\ge 10\text{pp}$ at the pre-registered horizon — an assumption, not a finding); AND (iii) task completion probability is non-inferior (within $5\text{pp}$ of the best baseline arm); AND (iv) mean cost per completed task plus latency (median + p95) are reported against proposed practicality triggers (pending pilot calibration — tripping a trigger yields INCONCLUSIVE, never a direct fail; see Table 6.2). Four quantities stay distinct: the MDID point estimate (depth reached here), its interval (precision), the CIF comparison (whether and how much failure incidence fell), and completion (whether success moved with it).
...
*   **Canonical comparison:** the thesis requires **(C) beats (B)** — GIBBRN-controlled versus conventionally controlled, same model/task/budget/run conditions where feasible. (A) unmanaged adaptive agent is a secondary reference that quantifies the problem severity, never the thesis comparison. Incremental value is measured as (C)−(B), never (C)−(A) alone. Where appropriate, (S) static deterministic pipeline is evaluated on the joint success–cost criterion.
*   **Null Hypothesis ($\mathcal{H}_0$):** gibbrn checkpointing and causal rollback do not jointly improve the failure–completion–depth profile versus conventional controls, or static deterministic pipelines (Agentless / pre-wired DAGs) achieve equal or superior completion rates at lower compute cost.
...
*   **Statistical Methodology:** Competing-risks primary: Gray's test on the fatal-failure CIF ($\alpha = 0.01$) with Fine-Gray subdistribution hazard regression for magnitude; completion CIF reported jointly. Descriptive: Kaplan-Meier MDID quantile with bootstrap CI for the MDID ratio (C vs B). The Gray's-test $p$-value tests *whether failure incidence differs*; the *magnitude* claims (≥2.0× depth, CIF effect floor) are evaluated via their intervals/estimates, not via any $p$-value. Sensitivity analyses include Kaplan-Meier + Log-Rank under the non-informative-censoring assumption and weighted log-rank tests (Peto-Peto and Harrington-Fleming $G^\rho$ family) for non-proportional hazards.
*   **Failure Threshold at Gate M18:** Verdicts follow canonical Table 6.2 — this summary restates it without modification: joint PROCEED requires the MDID ratio criterion AND the CIF reduction criterion AND completion non-inferiority AND no tripped practicality trigger (all C vs B). Partial satisfaction (e.g. depth met but CIF magnitude unconfirmed, or any straddling interval) → INCONCLUSIVE (more tasks/optimization); a tripped practicality trigger without a cost-reduction path → INCONCLUSIVE, not a pass. STOP/PIVOT only for: MDID ratio point <1.5, or CI upper bound <2.0 with adequate power, or no significant CIF reduction with adequate power, or completion inferior by $>5\text{pp}$, or static-pipeline win on the joint success–cost criterion. Incremental value is measured as (C)−(B), never (C)−(A) alone. (A) unmanaged and (S) static pipeline are secondary reports.
...
| **Core RQ6** | GAIA Level 3 & Deep SWE-bench | 150 deep tasks | Gray's CIF Test + Fine-Gray (KM/MDID descriptive) | 0.90 | 0.01 | Right-censoring at maximum step cutoffs; completion as competing event |
...
| M36 (RQ12 integrated) | M36-A scientific: composed surviving-metric targets met (frozen at M33 pre-reg) → thesis PROCEED | Partial composition → NARROW to responsive vertical | No integrated advantage → thesis PIVOT or STOP per investor governance. M36-B company (independent): ROI/adoption/integration-benefit targets met (frozen at M33 pre-reg) → company PROCEED | Mixed partner ROI → NARROW to responsive vertical | No ROI and no adoption path → company PIVOT or STOP per investor governance. A and B are verdict-independent. |
...
| M18 | Fatal-failure + completion CIFs, Gray's + Fine-Gray results, KM curves, MDID CIs, joint completion/depth/cost/latency table, no-op audit summary |
