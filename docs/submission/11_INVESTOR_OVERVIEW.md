...
Whether checkpoint/rollback doubles survival depth (MDDD₀.₉₀ ≥2.0×); whether regression-gated skill admission runs fast/cheap enough; whether developers adopt an out-of-process daemon; whether adaptations transfer or must be bounded (ATR); whether migration preserves mechanical continuity; whether objective integrity holds under adaptive optimization; whether Year-3 team/governance/state hypotheses survive conditional gates. Each maps to a kill/narrow threshold in Table 6.2 (RQ1–RQ7 numeric; RQ8–RQ12 calibrated at pilot) — frozen at pre-registration before data collection; nothing is preregistered yet.
